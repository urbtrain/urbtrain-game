...
export class Game {
  ...
  public constructor(canvas: HTMLCanvasElement) {
    ...
    this.camera = new FreeCamera("camera", new Vector3(0, 8.2, -18), this.scene); this.camera.setTarget(new Vector3(0, 1.5, 11)); this.camera.fov = 0.88;
  ...
  private update(dt: number): void {
    this.speed = Math.min(CONFIG.maxSpeed, CONFIG.initialSpeed + this.distance / 160); this.distance += this.speed * dt; this.score += this.speed * dt * 4 + this.medals * 0.002; this.track.update(dt, this.speed); this.runner.update(dt); this.camera.position.x += (this.runner.root.position.x * 0.48 - this.camera.position.x) * dt * 4; this.camera.position.y = 8.2; this.camera.position.z = -18; this.camera.setTarget(new Vector3(this.runner.root.position.x * 0.22, 1.5 + Math.sin(this.distance * 0.15) * 0.035, 11)); this.camera.fov = 0.88 + Math.min(0.08, (this.speed - CONFIG.initialSpeed) * 0.004);
